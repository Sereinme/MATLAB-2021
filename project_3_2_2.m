...
info_start = randi(max_length);
info_length = randi(max_length - info_start);
info_hide = randi([0, 1], 1, info_length);
info_bin = dec2bin(info_hide);
...
quant_bin(info_start:info_start + info_length - 1, end) = info_bin;
quant_hide = reshape(bin2dec(quant_bin), coeff_length, 64)';
...
info_extract = bin2dec(fill_extract(info_start:info_start + info_length - 1, end))';
...
% plot
imwrite(image, 'images/project_3_2_2.jpg');
subplot(1, 2, 1);
imshow(hall_gray);
title('Original');
subplot(1, 2, 2);
